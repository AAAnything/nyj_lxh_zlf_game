...
bool CollectSpot::init(const std::string& itemId, const std::string& tileType) {
    ...
    collected = false;
    currentClicks = 0;
    collectRange = 100.0f;               // 采集范围100像素

    // 获取GameManager实例
    auto gm = GameManager::getInstance();

    // 通过CollectManager获取采集物品（这里假设CollectManager有这个方法）
    ...
    if (item) {
        ...
        if (tileType == "tree") {
            spriteName = "tiles/tree.png";        // 树
        }
        else if (tileType == "rock") {
            spriteName = "tiles/rock.png";        // 石头
        }
        else if (tileType == "grass") {
            spriteName = "tiles/grass.png";       // 草
        }
        else if (tileType == "wood") {
            spriteName = "tiles/bush.png";        // 灌木
        }
        else {
